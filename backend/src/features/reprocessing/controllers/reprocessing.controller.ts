...
export class ReprocessingController {
  private service = new ReprocessingService();
  ...
  batch = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const data = await this.service.startBatch(req.body, req.user?.id!);
      res.status(201).json({ success: true, data });
    } catch (error) { next(error); }
  };
  ...
  retry = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const data = await this.service.retryJob(req.params.jobId, req.user?.id!);
      res.status(200).json({ success: true, data });
    } catch (error) { next(error); }
  };
}
