...
import { SectionPatternChart } from '../components/SectionPatternChart';
import { InternalChoiceAnalysis } from '../components/InternalChoiceAnalysis';
import { PatternComparison } from '../components/PatternComparison';
import { PatternStatistics } from '../components/PatternStatistics';
import { FiLayers, FiPlay, FiSearch, FiRefreshCw, FiCheck, FiX } from 'react-icons/fi';

export const PatternAnalysisDashboardPage: React.FC = () => {
    const [searchId, setSearchId] = useState('');
    const [activePaperId, setActivePaperId] = useState('');
    
    const { getStatus, processAnalysis, reprocessAnalysis, reviewAnalysis } = usePatternAnalysis(activePaperId);
        ...
        <div className="p-6 md:p-8 max-w-7xl mx-auto min-h-screen">
            <div className="mb-12 text-center">
                <h1 className="text-4xl font-black text-gray-900 dark:text-white flex items-center justify-center mb-4">
                    <span className="p-3 bg-gradient-to-br from-pink-500 to-rose-600 rounded-2xl mr-4 shadow-lg text-white">
                        <FiLayers size={32} />
                    </span>
                    Pattern & Blueprint Engine
                </h1>
                <p className="text-gray-600 dark:text-gray-400 text-lg max-w-2xl mx-auto font-medium">
                    Enterprise AI engine for detecting recurring structural layouts and generating exam blueprints.
                </p>
            </div>
                    ...
                    <div className="p-8">
                        {!statusData ? (
                            <div className="text-center py-16">
                                <p className="text-gray-500 font-medium mb-8 text-lg">Pattern analysis has not been performed on this document.</p>
                                <button 
                                    onClick={handleProcess}
                                    disabled={processAnalysis.isPending}
                                    className="px-10 py-5 bg-gradient-to-r from-pink-500 to-rose-600 hover:from-pink-600 hover:to-rose-700 text-white font-extrabold rounded-2xl shadow-lg transition-all hover:-translate-y-1 hover:shadow-xl disabled:opacity-50 inline-flex items-center text-lg"
                                >
                                    <FiPlay className="mr-3" size={24} /> Run Pattern Engine
                                </button>
                                <p className="text-sm text-gray-400 mt-4 italic">Requires previously extracted questions.</p>
                            </div>
                        ) : statusData.status === 'COMPLETED' ? (
                            <div>
                                <PatternOverview data={statusData} />
                                <PatternStatistics data={statusData} />
                                
                                <BlueprintViewer blueprint={statusData.blueprint} />
                                
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                                    <SectionPatternChart blueprint={statusData.blueprint} />
                                    <InternalChoiceAnalysis blueprint={statusData.blueprint} />
                                </div>

                                <div className="grid grid-cols-1 mb-8">
                                    <RecurringQuestions structures={statusData.recurringStructures} />
                                </div>
                                
                                <PatternComparison paperId={activePaperId} />
                                    ...
                                    {statusData.reviewStatus === 'PENDING_REVIEW' ? (
                                        <div className="flex gap-3">
                                            ...
                                            <button 
                                                ...
                                                className="px-6 py-3 bg-rose-600 text-white hover:bg-rose-700 shadow-md font-bold rounded-xl flex items-center transition-colors"
                                            >
                            ...
                            <div className="text-center py-24">
                                ...
                                <p className="text-gray-900 dark:text-white font-extrabold text-3xl mb-3">Detecting Structural Patterns...</p>
                                <p className="text-lg font-medium text-gray-500">Cross-referencing global blueprints and historical templates.</p>
                            </div>
                        )}
                    </div>
                </div>
