...
export const AnalysisQueue: React.FC<{ queue: any[], onSelect: (id: string) => void }> = ({ queue, onSelect }) => {
    if (!queue || queue.length === 0) {
        return (
            <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 text-center py-16">
                <FiCpu className="mx-auto text-gray-300 dark:text-gray-600 mb-5" size={48} />
                <p className="text-gray-500 dark:text-gray-400 font-bold text-lg">Pipeline queue is currently empty.</p>
                <p className="text-sm text-gray-400 mt-2">No documents pending analysis.</p>
            </div>
        );
    }
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div className="px-6 py-5 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 flex justify-between items-center">
                <h3 className="font-bold text-gray-900 dark:text-white flex items-center">
                    <FiActivity className="mr-3 text-indigo-500" size={20} /> Active Pipeline Queue
                </h3>
                <span className="px-3 py-1.5 bg-indigo-100 text-indigo-700 rounded-lg text-xs font-black shadow-sm">
                    {queue.length} JOBS
                </span>
            </div>
