...
export const PerformanceDashboardPage: React.FC = () => {
  ...
  const { summary, isSummaryLoading } = usePerformance();
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col">
      <header className="p-6 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 sticky top-0 z-10 flex justify-between items-center flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Monitor runtime efficiency, bundle sizes, and cache optimization.</p>
        </div>
        <div className="flex items-center gap-3 bg-gray-100 dark:bg-gray-800 px-4 py-2 rounded-lg">
           ...
           {isSummaryLoading ? (
              ...
              <span className={`text-sm font-bold \${summary?.status === 'OPTIMIZED' ? 'text-green-600' : 'text-orange-600'}`}>
                {summary?.status}
              </span>
           )}
        </div>
      ...
      <div className="flex gap-2 overflow-x-auto p-4 shrink-0 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
        {['RUNTIME', 'MEMORY_MONITOR', 'BUNDLE_ANALYSIS', 'LAZY_LOADING', 'IMAGE_OPTIMIZATION', 'CACHE', 'NETWORK', 'RECOMMENDATIONS'].map(tab => (
          <button
            key={tab}
